...
void worker(chanend fromDistr,int workerNumber) {
    ...
    uchar val;
    for (int x = 0; x < 1; x++) { //no of iterations of game of life
        for (int i = 0; i < IMHT; i++) {
...
//  unsigned int endTime;
  ...
  uchar partOfGrid[IMHT][IMWD/4 + 2];
  for (int a = 0; a < 1; a++) { //no of iterations in game of life
      for(int i = 0; i<4;i++){
          //if(i==0) t:>startTime;
...
par {
    ...
    on tile[0] : DataInStream("7th.pgm", c_inIO);          //thread to read in a PGM image
